...
LinkedList removeDupplicateValue(LinkedList linkedList)
{
    LinkedList result = init();
    Node *current = linkedList.head;
    while (current != nullptr)
    {
        if (find(result, current->value) == -1)
            addTail(result, current->value);
        current = current->next;
    }
    return result;
}
...
int main(int argc, char const *argv[])
{
    ...
    linkedList = removeDupplicateValue(linkedList);
    printOutList(linkedList);
    return 0;
}
